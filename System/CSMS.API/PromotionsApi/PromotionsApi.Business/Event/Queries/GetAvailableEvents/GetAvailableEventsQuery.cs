...
{
    public class GetAvailableEventsQuery : IGetAvailableEventsQuery
    {
        private readonly IRepository<CsmsEvent> _eventRepository;

        public GetAvailableEventsQuery(IRepository<CsmsEvent> eventRepository)
        {
            _eventRepository = eventRepository;
        }

        public async Task<List<AvailableEventViewModel>> ExecuteAsync()
        {
            var result = await _eventRepository.TableNoTracking
                .Where(n =>
                    (n.Code == null) &&
                    (n.StartTime <= DateTime.Now) &&
                    (n.EndTime != null ? n.EndTime >= DateTime.Now : true) &&
                    (n.Enabled == true)
                )
